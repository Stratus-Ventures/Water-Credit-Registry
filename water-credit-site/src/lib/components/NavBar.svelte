<script lang="ts">
	...
	const activeIndex = $derived(tabs.findIndex((t) => page.url.pathname === t.page));
</script>

<nav
	class="
    fixed right-0 bottom-8 left-0 z-40 mx-auto
    flex h-fit w-fit flex-row
    items-center justify-center rounded-full bg-button-bg/80 p-1
    text-inverted-primary-fg shadow-card backdrop-blur-xl
    sm:bottom-16 dark:bg-frosted-bg dark:shadow-none"
>
	...
	<div class="absolute inset-0 overflow-hidden rounded-full" aria-hidden="true">
		...
		<span
			class="pointer-events-none absolute top-1 left-1 size-13 rounded-full bg-inverted-tertiary-bg
    		       duration-base ease-snappy will-change-transform sm:size-10 dark:bg-tertiary-bg
	...
	<div class="relative flex h-fit w-fit flex-row gap-1.5">
		{#each tabs as tab, i (tab.id)}
			{@const active = i === activeIndex}

			<button
				class="
                    group relative z-10 grid size-13 place-items-center
                    rounded-full transition-colors duration-fast ease-snappy focus-within:z-30
                    hover:z-30 active:z-30 sm:size-10
                    {active
					? 'text-inverted-primary-fg dark:text-primary-fg'
					: 'text-inverted-secondary-fg hover:text-inverted-primary-fg hover:dark:text-primary-fg'}"
